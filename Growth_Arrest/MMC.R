...
MMC.PDL<-ggplot(data=s2,aes(x=characteristics..population_doublings,y=med))+geom_smooth(method='lm',alpha=0.4,colour="darkgray")+
  geom_point(aes(colour=characteristics..culture_agent),alpha=0.6)+
scale_color_manual(name="Agent",values=c(control='maroon',MMC='steelblue4'))+
  ...
  facet_wrap(~characteristics..CORIELL_ID,scales='free_x',labeller = labeller('Cell.line' = c("AG16146"="Adult Skin Fibroblast \nAG16146",
...
MMC.days<-ggplot(data=s2,aes(x=characteristics..days_in_culture,y=med,col=characteristics..culture_agent))+geom_point(alpha=0.6)+
   scale_color_manual(name="Agent",values=c(control='maroon',MMC='steelblue4'))+
...
MMC.growthcurve<-ggplot(data=s2,aes(y=characteristics..population_doublings,x=characteristics..days_in_culture,col=characteristics..culture_agent))+
